...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (itemCost.error) {
    throw redirect(
      ...
      await flash(
        request,
        error(itemCost.error, "Failed to load fixture costing")
      )
    );
  }
  if (accounts.error) {
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (updateItemCost.error) {
    throw redirect(
      ...
      await flash(
        request,
        error(updateItemCost.error, "Failed to update fixture costing")
      )
    );
  }

  throw redirect(
    path.to.fixtureCosting(itemId),
    await flash(request, success("Updated fixture costing"))
  );
}

export default function FixtureCostingRoute() {
