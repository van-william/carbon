...
const NonConformanceWorkflowForm = ({
  initialValues,
  onClose,
}: NonConformanceWorkflowFormProps) => {
  ...
  return (
    <ValidatedForm
      ...
      validator={nonConformanceWorkflowValidator}
      defaultValues={initialValues}
      method="post"
      action={
        isEditing
          ? path.to.nonConformanceWorkflow(initialValues.id!)
          : path.to.newNonConformanceWorkflow
      }
    >
      ...
      >
        <HStack className="w-full justify-between">
          <VStack spacing={0}>
            <Heading size="h3">
              {isEditing ? "Edit" : "New"}{" "}
              <span className="hidden md:inline">Non-Conformance</span> Workflow
            </Heading>
            <p className="text-sm text-muted-foreground">
              Non-conformance workflows are used as a starting point for issues.
              Each non-conformance workflow uses a workflow to create the issue.
              A single workflow can be used in multiple workflows.
            </p>
          </VStack>
...
export default NonConformanceWorkflowForm;
