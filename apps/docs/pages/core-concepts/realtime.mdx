...
description: Conventions for using realtime data in CarbonOS
---

# Realtime

There are two basic use cases for realtime data in CarbonOS:
...
In practice, however, this approach is too heavy-handed. A better idea is to listen to smaller subset of the table can be listened to by passing a second argument. Here we listen for the rows with an `id` of `1`, `2`, or `3`:
