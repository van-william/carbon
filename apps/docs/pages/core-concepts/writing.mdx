---
title: Data Writes in CarbonOS
description: Conventions for mutating data in CarbonOS
---
...
```ts
import { withZod } from "@carbon/form";
import { z } from "zod";
...
interface DB {
  ...
  itemLedger: Database["public"]["Tables"]["itemLedger"]["Insert"];
...
serve(async (req: Request) => {
  try {
    ...
    await db.transaction().execute(async (trx) => {
      await trx
        .updateTable("itemLedger")
        .set({
          expectedCost: 0,
        })
        .where("id", "=", itemLedgerId)
        .execute();
