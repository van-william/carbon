-- Add thumbnailPath column to item table
ALTER TABLE "item"
ADD COLUMN "thumbnailPath" TEXT;

-- Update item table with thumbnailPath from modelUpload
UPDATE "item" i
SET "thumbnailPath" = mu."thumbnailPath"
FROM "modelUpload" mu
WHERE i."modelUploadId" = mu.id;

-- Update views to include the new thumbnailPath from item table
...
CREATE POLICY "Employees can view item thumbnails" ON storage.objects 
FOR SELECT USING (
    bucket_id = 'private'
...
CREATE POLICY "Thumbnail insert requires parts_update" ON storage.objects 
...
CREATE POLICY "Thumbnail update requires parts_update" ON storage.objects 
FOR UPDATE USING (
    bucket_id = 'private'
    AND has_role('employee', (storage.foldername(name))[1])
    AND has_company_permission('parts_update', (storage.foldername(name))[1])
...
CREATE POLICY "Thumbnail delete requires parts_delete" ON storage.objects 
FOR DELETE USING (
    bucket_id = 'private'
    AND has_role('employee', (storage.foldername(name))[1])
